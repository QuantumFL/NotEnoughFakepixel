...
package org.ginafro.notenoughfakepixel.core;
...
public class AutoRoom {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (tickAmount % 30 == 0 && ScoreboardUtils.currentLocation.isDungeon() && worldLoad == 200) {
            executor.execute(() -> {
                if (AutoRoom.chatToggled || AutoRoom.guiToggled || Configuration.dungeonsSecretsWaypoints){
    ...
    public void onWorldChange(WorldEvent.Load event) {
        ...
        if (random.nextBoolean()) {
            if (NotEnoughFakepixel.motd != null) {
                if (!NotEnoughFakepixel.motd.isEmpty()) {
                    output.addAll(NotEnoughFakepixel.motd);
                }
            }
        }
    ...
    public static List<String> autoText() {
        ...
        String text = "You are in " + EnumChatFormatting.GREEN;
        ...
        if (NotEnoughFakepixel.roomsJson.get(MD5) == null && Utils.getSize(x,top,z).equals("1x1")) {
            output.add(EnumChatFormatting.LIGHT_PURPLE + "FDR: If you see this message in game (and did not create ghost blocks), send a");
            ...
            output.add("FDR: You are probably in: ");
            output.add(EnumChatFormatting.GREEN + "Literally no idea, all the rooms should have been found");
            lastRoomJson = null;
            return output;
        } else if (NotEnoughFakepixel.roomsJson.get(MD5) == null) {
            lastRoomJson = null;
            return output;
        }

        JsonArray MD5Array = NotEnoughFakepixel.roomsJson.get(MD5).getAsJsonArray();
        ...
        if (arraySize >= 2) {
            ...
            for(int i = 0; i < arraySize; i++){
                ...
                if (floorHash != null && jsonFloorHash != null){
                    if (floorHash.equals(jsonFloorHash.getAsString())){
                        ...
                        if (NotEnoughFakepixel.waypointsJson.get(name) == null && secrets != 0 && Configuration.dungeonsSecretsWaypoints) {
            ...
            if (!floorHashFound) {
                output.add("FDR: You are probably in one of the following: ");
